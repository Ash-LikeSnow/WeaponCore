    ...
    internal class ApiBackend
    {
        ...
        internal ApiBackend()
        {
            ...
            ModApiMethods = new Dictionary<string, Delegate>
            {
                ...
                ["GetAllSmartProjectiles"] = new Action<ICollection<MyTuple<ulong, Vector3D, int>>>(GetAllSmartProjectiles),
        ...
        private void GetAllSmartProjectiles(ICollection<MyTuple<ulong, Vector3D, int>> collection)
        {
            collection.Clear();
            foreach (var p in Session.I.Projectiles.ActiveProjetiles)
            {
                ...
                {
                    collection.Add(new MyTuple<ulong, Vector3D, int>(p.Info.Id, p.Position, p.Info.Age));
                }
            }
            return;
        }
